...
<<<<<<< HEAD
| dotType | default | `string` | When `total` is greater than 7, you can set it to `instagram`. This property allows the dots to be slipped and faded. When its value is `instagram`, the prop `loop` must be set to `false`. |
=======
