...
export default class extends Component {
  ...
  initState(props, updateIndex = false) {
    ...
    const initState = { autoplayEnd: false, loopJump: false, offset: {} };
  ...
  autoplay = () => {
    ...
    this.autoplayTimer = setTimeout(() => {
      const isAutoplayEnded = this.props.autoplayDirection
        ? this.state.index === this.state.total - 1
        : this.state.index === 0;

      if (!this.props.loop && isAutoplayEnded) {
  ...
  onScrollEnd = (e) => {
    ...
    if (!e.nativeEvent.contentOffset) {
      e.nativeEvent.contentOffset = this.state.dir === 'x'
   ...
   * Drag end handle.
   *
   * @param {object} contentOffset Content offset
   */
  ...
  renderPagination = () => {
    ...
    for (let key = 0; key < this.state.total; key += 1) {
      dots.push(React.cloneElement(key === this.state.index ? ActiveDot : Dot, { key }));
    }
  ...
  refScrollView = (view) => { this.scrollView = view; };
